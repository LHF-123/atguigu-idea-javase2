 ...
 *  2、当(形参列表)是非空参时，并且类型是确定的或者可以推断的，那么形参的数据类型可以省略
 *  3、当(形参列表)是非空参时，并且只有一个形参，并且类型也省略了，那么此时()也可以省略，如果类型没有省略，那么()也不能省略
 *  4、如果{lambda体}不止一个语句，那么{}不能省略，并且每一个语句都要;结束
 *  5、如果{lambda体}只有一个语句，那么{}和;都可以省略，{}和;要同时省略
 *  6、如果该函数式接口的抽象方法的返回值不是void，那么{lambda体}里面要有return 返回值语句;
 *      当如果{lambda体}中只有一个{return 返回值;}语句时，此时{}和;和return都同时省略。
 *
 */
...
//        TreeSet<Employee> set = new TreeSet<>((Employee o1, Employee o2) -> {
