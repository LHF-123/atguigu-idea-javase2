 ...
 *  （5）getInterface()
 *  （6）getField
 *      Field getField(String name):返回一个 Field 对象，此方法返回该类及其所有超类的公共字段。
 *      Field[] getFields():返回一个包含某些 Field 对象的数组，这些对象反映此 Class 对象所表示的类或接口的所有可访问公共字段。
 *       Field[] getDeclaredFields()：返回一个 Field 对象，该对象反映此 Class 对象所表示的类或接口的指定已声明字段。
 *                                          包括公共、保护、默认（包）访问和私有字段，但不包括继承的字段。
 *      Field getDeclaredField(String name):返回指定的 Field 对象，该对象反映此 Class 对象所表示的类或接口的指定已声明字段。
 *                                          包括公共、保护、默认（包）访问和私有字段，但不包括继承的字段。
 ...
 *  （2）Modifier类型
 *  （3）Field类型
 *      Field代表属性，每一个Field的对象代表某一个类中的某一个属性。
 *      所有的属性有共同特点：
 *          【修饰符】数据类型 属性名;
 *          get/set值
 *
